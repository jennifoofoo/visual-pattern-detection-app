...
class ClusterPattern(Pattern):
    ...
    def _calculate_dynamic_embedding_params(self, df: pd.DataFrame, column: str) -> tuple:
        ...
        Small dataset: 50 rows, 8 activities тЖТ embed_dim=5, max_features=5
        Medium dataset: 1000 rows, 25 activities  тЖТ embed_dim=10, max_features=7
        Large dataset: 50000 rows, 200 activities тЖТ embed_dim=25, max_features=60
    ...
    def detect(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            self.detected = {
                'labels': labels,
                'coordinates': X_scaled,  # Use scaled coordinates for silhouette score
    ...
    def visualize(self, df: pd.DataFrame, fig: go.Figure, selected_clusters: list = None, show_noise: bool = False) -> go.Figure:
        ...
        selected_clusters : list, optional
            List of cluster IDs to display. If None, shows all clusters.
        ...
        for i, label in enumerate(unique_labels):
            ...
            unique_activities = cluster_data[y_col].nunique(
            ) if y_col in cluster_data.columns else 0
            unique_cases = cluster_data['case_id'].nunique(
            ) if 'case_id' in cluster_data.columns else 0
    ...
    def get_cluster_summary(self) -> Dict[str, Any]:
        ...
        # Requires: more than 1 cluster and coordinates available (not hierarchical)
        summary['silhouette_score'] = None  # Default to None
        X = self.detected.get('coordinates')

        if X is not None and len(unique_labels) > 1:
            try:
                clustered_mask = labels >= 0
                n_clustered = np.sum(clustered_mask)

                # Ensure X and labels have matching lengths
                if len(X) == len(labels) and n_clustered > 1:
                    X_clustered = X[clustered_mask]
                    labels_clustered = labels[clustered_mask]

                    # Need at least 2 different labels for silhouette
                    unique_clustered_labels = np.unique(labels_clustered)
                    if len(unique_clustered_labels) > 1:
                        score = silhouette_score(X_clustered, labels_clustered)
                        summary['silhouette_score'] = float(score)
                else:
                    print(
                        f"Silhouette: X length {len(X)} != labels length {len(labels)}")
            except Exception as e:

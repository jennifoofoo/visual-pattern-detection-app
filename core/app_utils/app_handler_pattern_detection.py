        ...
        with subtab2:
            ...
            if hasattr(detector, 'clusters') and 'temporal_bursts' in detector.clusters:
                ...
                if selected_clusters:
                    st.success(
                        f"✅ {len(selected_clusters)} of {len(cluster_list)} clusters selected")
# endregion
...
def display_outlier_tab():
    ...
    if st.session_state.get('outlier_detected', False) and 'outlier_pattern' in st.session_state:
        ...
        view_count = summary.get('count', 0)
        total_count = summary.get('total_count', view_count)

        # Show appropriate message based on view relevance
        if view_count > 0:
            st.success(
                f"{view_count} outliers relevant to this view ({stats.get('outlier_percentage', 0):.1f}%)")
        elif total_count > 0:
            st.info(
                f"No outliers visible in current view. {total_count} outliers found in other views (try changing Y-axis or color).")
        else:
            st.info("No significant outliers detected.")
        ...
        with subtab2:
            ...
            if outlier_details:
                ...
                if selected_types:
                    st.success(
                        f"✅ {len(selected_types)} of {len(outlier_types_dict)} outlier types selected")
            else:
    ...
    if 'gap_detector' in st.session_state and st.session_state['gap_detector'].detected is not None:
        ...
        with col_info:
            ...
            if not layer_visible:
                ...
                st.success(
                    f"{summary['count']} gaps detected across {details['transitions_with_anomalies']} transitions")
        with col_settings:
            with st.popover("⚙️"):
                st.write("**Settings**")
                current_min_samples = st.session_state.get(
                    'gap_min_samples', 5)
                ...
                if min_samples != current_min_samples:
                    if st.button("Apply & Re-detect", use_container_width=True, type="primary", key="gap_redetect_tab"):
                        ...
                        plot_config = st.session_state.get(
                            'current_plot_config', {})
                        if plot_config:
                            ...
                            _detect_gaps(
                                x_col, y_col, df_selected, min_samples)
                            st.rerun()
        ...
        with subtab2:
            ...
            if trans_stats:
                ...
                if selected_transitions:
                    st.success(
                        f"✅ {len(selected_transitions)} of {len(transition_dict)} transitions selected")
            else:
        ...
        with subtab2:
            ...
            if pattern_stats:
                ...
                if selected_patterns:
                    st.success(
                        f"✅ {len(selected_patterns)} of {len(sequence_dict)} sequences selected")
            else:
